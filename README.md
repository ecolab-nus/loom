...
open the cloned repository in VS Code, and run **Dev Containers: Reopen in Container** from the Command Palette. When prompted, select `standard` for a
non-Tenstorrent machine, or `tenstorrent` to attach a Tenstorrent device.
...
The input file, `test/matmul_2Dmesh/IRs/p03_bufferized.mlir`, is generated by
the previous step. The resulting Tenstorrent kernel code is written to
`tmp_output/kernels`. The final argument, `1`, limits lowering to the first
function.
