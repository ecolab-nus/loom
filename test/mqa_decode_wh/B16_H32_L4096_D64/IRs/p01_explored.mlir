        ...
        affine.parallel (%arg5) = (0) to (8) {
          ...
          scf.for %arg6 = %c0 to %25 step %c1 {
            ...
            scf.for %arg7 = %c0 to %26 step %c1 {
              ...
              %30 = loom.alloc [%20, 32, 64] on @L1 : memref<?x32x64xf16>
              %31 = loom.semaphore_take %30 : memref<?x32x64xf16> -> memref<?x32x64xf16>
              %32 = loom.init_tensor %31[%20, 32, 64] : memref<?x32x64xf16> -> tensor<?x32x64xf16>
              ...
              %40 = loom.alloc [%20, 32, 64] on @L1 : memref<?x32x64xf16>
              %41 = loom.semaphore_take %40 : memref<?x32x64xf16> -> memref<?x32x64xf16>
              %42 = loom.init_tensor %41[%20, 32, 64] : memref<?x32x64xf16> -> tensor<?x32x64xf16>
              %43 = loom.semaphore_take %40 : memref<?x32x64xf16> -> memref<?x32x64xf16>
              %44 = loom.init_tensor %43[%20, 32, 64] : memref<?x32x64xf16> -> tensor<?x32x64xf16>
              %45 = loom.semaphore_take %40 : memref<?x32x64xf16> -> memref<?x32x64xf16>
              %46 = loom.init_tensor %45[%20, 32, 64] : memref<?x32x64xf16> -> tensor<?x32x64xf16>
              %47 = linalg.fill ins(%cst : f16) outs(%46 : tensor<?x32x64xf16>) -> tensor<?x32x64xf16>
              ...
              %64 = loom.alloc [%20, 32, 64] on @L1 : memref<?x32x64xf16>
              %65 = loom.semaphore_take %64 : memref<?x32x64xf16> -> memref<?x32x64xf16>
              %66 = loom.init_tensor %65[%20, 32, 64] : memref<?x32x64xf16> -> tensor<?x32x64xf16>
              ...
              %90:3 = scf.for %arg8 = %c0 to %39 step %c1 iter_args(%arg9 = %63, %arg10 = %57, %arg11 = %47) -> (tensor<?x32x1xf16>, tensor<?x32x1xf16>, tensor<?x32x64xf16>) {
                ...
                %119 = linalg.batch_matmul ins(%37, %117 : tensor<?x32x64xf16>, tensor<?x64x?xf16>) outs(%118 : tensor<?x32x?xf16>) -> tensor<?x32x?xf16>
                ...
                %133 = linalg.batch_matmul ins(%124, %131 : tensor<?x32x?xf16>, tensor<?x?x64xf16>) outs(%132 : tensor<?x32x64xf16>) -> tensor<?x32x64xf16>
                loom.semaphore_give %89 : memref<?x?x64xf16>
                loom.semaphore_give %79 : memref<?x32x?xf16>
                %134 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%133, %arg11, %129 : tensor<?x32x64xf16>, tensor<?x32x64xf16>, tensor<?x32x64xf16>) outs(%arg11 : tensor<?x32x64xf16>) {
                ^bb0(%in: f16, %in_3: f16, %in_4: f16, %out: f16):
                ...
                loom.semaphore_give %65 : memref<?x32x64xf16>
                ...
                scf.yield %135, %128, %134 : tensor<?x32x1xf16>, tensor<?x32x1xf16>, tensor<?x32x64xf16>
              } {loom.iter_type = #loom.iter_type<sequential>}
              loom.semaphore_give %35 : memref<?x32x64xf16>
              ...
              %92 = loom.broadcast ins(%90#1 : tensor<?x32x1xf16>) outs(%83 : tensor<?x32x32xf16>) dim(2) -> tensor<?x32x64xf16>
              loom.semaphore_give %55 : memref<?x32x1xf16>
              %93 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%90#2, %92 : tensor<?x32x64xf16>, tensor<?x32x64xf16>) outs(%34 : tensor<?x32x64xf16>) {
              ^bb0(%in: f16, %in_3: f16, %out: f16):
                %114 = arith.divf %in, %in_3 : f16
                linalg.yield %114 : f16
              } -> tensor<?x32x64xf16>
              loom.semaphore_give %82 : memref<?x32x32xf16>
              loom.semaphore_give %45 : memref<?x32x64xf16>
              ...
              %99 = loom.sync ins(%93 : tensor<?x32x64xf16>) outs(%44 : tensor<?x32x64xf16>) -> tensor<?x32x64xf16>
              loom.semaphore_give %33 : memref<?x32x64xf16>
              %100 = loom.alloc [%24, %20, 32, 64] on @L1 : memref<?x?x32x64xf16>
              %101 = loom.semaphore_take %100 : memref<?x?x32x64xf16> -> memref<?x?x32x64xf16>
              %102 = loom.init_tensor %101[%24, %20, 32, 64] : memref<?x?x32x64xf16> -> tensor<?x?x32x64xf16>
              %103 = loom.gather ins(%99 : tensor<?x32x64xf16>) outs(%102 : tensor<?x?x32x64xf16>) across(%arg5 : index) region : (UL : [%c0, %arg4], LR : [%c7, %arg4]) -> tensor<?x?x32x64xf16>
              loom.semaphore_give %43 : memref<?x32x64xf16>
              ...
              %107 = loom.alloc [%24, %20, 32, 64] on @L1 : memref<?x?x32x64xf16>
              %108 = loom.semaphore_take %107 : memref<?x?x32x64xf16> -> memref<?x?x32x64xf16>
              %109 = loom.init_tensor %108[%24, %20, 32, 64] : memref<?x?x32x64xf16> -> tensor<?x?x32x64xf16>
              ...
              scf.if %113 {
                ...
                %121 = loom.broadcast ins(%120 : tensor<?x?x32x1xf16>) outs(%112 : tensor<?x?x32x32xf16>) dim(3) -> tensor<?x?x32x64xf16>
                loom.semaphore_give %105 : memref<?x?x32x1xf16>
                %122 = loom.sync ins(%103 : tensor<?x?x32x64xf16>) outs(%109 : tensor<?x?x32x64xf16>) -> tensor<?x?x32x64xf16>
                loom.semaphore_give %101 : memref<?x?x32x64xf16>
                %123 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%122, %121 : tensor<?x?x32x64xf16>, tensor<?x?x32x64xf16>) outs(%109 : tensor<?x?x32x64xf16>) {
                ^bb0(%in: f16, %in_3: f16, %out: f16):
                  %129 = arith.mulf %in, %in_3 : f16
                  linalg.yield %129 : f16
                } -> tensor<?x?x32x64xf16>
                loom.semaphore_give %111 : memref<?x?x32x32xf16>
                %124 = linalg.fill ins(%cst : f16) outs(%32 : tensor<?x32x64xf16>) -> tensor<?x32x64xf16>
                %125 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d1, d2, d3)>], iterator_types = ["reduction", "parallel", "parallel", "parallel"]} ins(%123 : tensor<?x?x32x64xf16>) outs(%124 : tensor<?x32x64xf16>) {
                ^bb0(%in: f16, %out: f16):
                  %129 = arith.addf %in, %out : f16
                  linalg.yield %129 : f16
                } -> tensor<?x32x64xf16>
