  ...
  func.func @_mqa_decode(%arg0: memref<16x64x4096xf16>, %arg1: memref<16x4096x64xf16>, %arg2: memref<16x32x64xf16>, %arg3: memref<16x32x64xf16>) {
    ...
    affine.parallel (%arg4, %arg5) = (0, 0) to (symbol(%3), symbol(%4)) {
      ...
      %16:3 = scf.for %arg6 = %c0 to %15 step %c1 iter_args(%arg7 = %6, %arg8 = %7, %arg9 = %9) -> (tensor<?x32x1xf16>, tensor<?x32x1xf16>, tensor<?x32x64xf16>) {
        ...
        %34 = linalg.batch_matmul ins(%11, %29 : tensor<?x32x64xf16>, tensor<?x64x?xf16>) outs(%33 : tensor<?x32x?xf16>) -> tensor<?x32x?xf16>
        ...
        %49 = "loom.broadcast"(%46, %38) {dim = 2 : i64} : (tensor<?x32x1xf16>, tensor<?x32x32xf16>) -> tensor<?x32x64xf16>
        %50 = linalg.generic {indexing_maps = [#map, #map, #map], iterator_types = ["parallel", "parallel", "parallel"]} ins(%arg9, %49 : tensor<?x32x64xf16>, tensor<?x32x64xf16>) outs(%8 : tensor<?x32x64xf16>) {
        ^bb0(%in: f16, %in_5: f16, %out: f16):
          %56 = arith.mulf %in, %in_5 : f16
          linalg.yield %56 : f16
        } -> tensor<?x32x64xf16>
        ...
        %54 = linalg.batch_matmul ins(%42, %51 : tensor<?x32x?xf16>, tensor<?x?x64xf16>) outs(%9 : tensor<?x32x64xf16>) -> tensor<?x32x64xf16>
        %55 = linalg.generic {indexing_maps = [#map, #map, #map], iterator_types = ["parallel", "parallel", "parallel"]} ins(%54, %50 : tensor<?x32x64xf16>, tensor<?x32x64xf16>) outs(%8 : tensor<?x32x64xf16>) {
        ^bb0(%in: f16, %in_5: f16, %out: f16):
          %56 = arith.addf %in, %in_5 : f16
          linalg.yield %56 : f16
        } -> tensor<?x32x64xf16>
        scf.yield %37, %48, %55 : tensor<?x32x1xf16>, tensor<?x32x1xf16>, tensor<?x32x64xf16>
      }
      ...
      %20 = "loom.broadcast"(%16#1, %19) {dim = 2 : i64} : (tensor<?x32x1xf16>, tensor<?x32x32xf16>) -> tensor<?x32x64xf16>
      %21 = linalg.generic {indexing_maps = [#map, #map, #map], iterator_types = ["parallel", "parallel", "parallel"]} ins(%16#2, %20 : tensor<?x32x64xf16>, tensor<?x32x64xf16>) outs(%8 : tensor<?x32x64xf16>) {
      ^bb0(%in: f16, %in_3: f16, %out: f16):
      ...
      %24 = tensor.empty(%4, %0) : tensor<?x?x32x64xf16>
      %25 = "loom.gather"(%21, %24, %arg5) {operandSegmentSizes = array<i32: 1, 1, 1, 0, 0, 0, 0>} : (tensor<?x32x64xf16>, tensor<?x?x32x64xf16>, index) -> tensor<?x?x32x64xf16>
      ...
      scf.if %26 {
        ...
        %36 = "loom.broadcast"(%34, %35) {dim = 3 : i64} : (tensor<?x?x32x1xf16>, tensor<?x?x32x32xf16>) -> tensor<?x?x32x64xf16>
        %37 = arith.cmpi eq, %4, %1 : index
        cf.assert %37, "mismatched size for broadcast"
        %38 = linalg.generic {indexing_maps = [#map2, #map2, #map2], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%25, %36 : tensor<?x?x32x64xf16>, tensor<?x?x32x64xf16>) outs(%24 : tensor<?x?x32x64xf16>) {
        ^bb0(%in: f16, %in_4: f16, %out: f16):
          %41 = arith.mulf %in, %in_4 : f16
          linalg.yield %41 : f16
        } -> tensor<?x?x32x64xf16>
        %39 = linalg.generic {indexing_maps = [#map2, #map3], iterator_types = ["reduction", "parallel", "parallel", "parallel"]} ins(%38 : tensor<?x?x32x64xf16>) outs(%9 : tensor<?x32x64xf16>) {
        ^bb0(%in: f16, %out: f16):
          %41 = arith.addf %in, %out : f16
          linalg.yield %41 : f16
        } -> tensor<?x32x64xf16>
